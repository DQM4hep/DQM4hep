...
\begin{figure}[htbp]
  \begin{center}
    \includegraphics[width=0.8\linewidth]{DQM4HEPArchitecture.pdf}
...
To control the data acquisition the DAQ system sends start/stop/status signals. These signals can be received by the framework \emph{Run Control Server}, and then dispatched to the DQM applications. Starting (stopping) a new run from the DAQ system will automatically start (stop) the DQM applications.
{\color{red}For now the Run Control Server receive such signals from the DAQ through HTTP POST and dispatch it with DIM.}

Second coupling to the DAQ system is the \emph{event builder}. Its purposes is to get the data from the  DAQ system (e.g. from a shared memory space), reconstruct this data to an user defined event structure before sending this event to event collectors using DIM. 
\begin{figure}[htbp]
  \begin{center}
    \includegraphics[width=0.8\linewidth]{DQMEventBuilder.pdf}
    \caption{\label{fig:DQMEventBuilder} The event builder}
  \end{center}
\end{figure}
...
As can be seen on Fig.~\ref{fig:DQMDataAccess}, data from these collectors can then be accessed either via direct user query (i.e. modules send a query for new data to the collector) or via subscription ({\color{red}push}) system. In the later 
case, collectors publish newly available data to the modules.

\begin{figure}[htbp]
  \begin{center}
    \includegraphics[width=0.8\linewidth]{DQMDataAccess.pdf}
...
Three GUIs are designed into the framework, each one with a dedicated role. The main interface is the 
\begin{figure}[p]
  \begin{center}
    \includegraphics[width=0.8\linewidth]{DQMMainViz.pdf}
...
Remote control of applications: Job Interface
Multiple applications deployed over multiple hosts Need for an easy way of controlling them
...
{\color{red}phrase de mes deux a reformuler growThe framework was developed within the CALICE\footnote{CAlorimeter for LInear Collider Experiment}-SDHCAL\footnote{Semi Digital Hadronic CALorimeter} Collaboration and thus a dedicated implementation has been developed in parallel to help us monitor the detector during test beam campaigns. A dedicated implementation for the CALICE-SiWEcal\footnote{Silicon (W)Tungsten Electronic calorimeter)} prototype was also developed for the purpose of a combined test beam with the two prototypes.}

- Event building - levbdim
- Serialization and event type definition based on the LCIO[^4] Event Data Model (xdrlcio)
- Multiple online analysis modules (Event display, slow control, detector efficiency, beam information, etc.)


Successfully used both with offline setup and during two Test Beam campaigns at CERN
Deployment over 4 computers
Shifters were able to quickly spot multiple problems:
- High current
- Bad gas circulation
- Undesirably noisy electronics
- Wrong beam configuration, etc.
