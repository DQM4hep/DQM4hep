...
// -- dqm4hep headers
#include "dqm4hep/DimEventCollector.h"
...
namespace dqm4hep {
  ...
  namespace core {
    ...
    static const char DimEventCollector_emptyBuffer [] = "EMPTY";
    static const uint32_t DimEventCollector_emptyBufferSize = 5;

    DimEventRequestRpc::DimEventRequestRpc(DimEventCollector *pCollector) :
    ...
    DimEventCollector::DimEventCollector() :
    ...
    StatusCode DimEventCollector::setCollectorName(const std::string &collectorName)
    {
      if(isRunning())
    ...
    const std::string &DimEventCollector::getCollectorName() const
    {
    ...
    bool DimEventCollector::isRunning() const
    {
      return m_isRunning;
    }
    ...
    StatusCode DimEventCollector::startCollector()
    {
      if(this->isRunning())
        return STATUS_CODE_SUCCESS;
      ...
      m_pEventUpdateService = new DimService(("DQM4HEP/EventCollector/" + getCollectorName() + "/EVENT_RAW_UPDATE").c_str(), "C",
          (void*) &DimEventCollector_emptyBuffer[0], DimEventCollector_emptyBufferSize);

      m_pStatisticsService = new StatisticsService("DQM4HEP/EventCollector/" + getCollectorName() + "/STATS");
    ...
    StatusCode DimEventCollector::stopCollector()
    {
      if(!this->isRunning())
        return STATUS_CODE_SUCCESS;
    ...
    void DimEventCollector::setEventStreamer(EventStreamer *pEventStreamer)
    {
      if(m_pEventStreamer)
        delete m_pEventStreamer;
    ...
    EventStreamer *DimEventCollector::getEventStreamer() const
    {
    ...
    void DimEventCollector::handleEventReception(DimCommand *pDimCommand)
    {
    ...
    void DimEventCollector::handleEventRequest(DimEventRequestRpc *pDimRpc)
    {
      ...
      if(NULL != m_pEventStreamer && NULL != m_pCurrentEvent && !subEventIdentifier.empty())
      {
        ...
        pDimRpc->setData((void *) &DimEventCollector_emptyBuffer[0], DimEventCollector_emptyBufferSize);
    }

    //-------------------------------------------------------------------------------------------------

    DimEventCollector::Client &DimEventCollector::getClient(int clientId)
    {
    ...
    void DimEventCollector::commandHandler()
    {
    ...
    void DimEventCollector::clientExitHandler()
    {
    ...
    void DimEventCollector::updateEventService()
    {
    ...
    void DimEventCollector::removeClient(int clientId)
    {
      if(clientId < 0)
        return;
    ...
    xdrstream::BufferDevice *DimEventCollector::configureBuffer(char *pBuffer, xdrstream::xdr_size_t bufferSize)
    {
