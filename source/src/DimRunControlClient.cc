...
namespace dqm4hep {
  ...
  namespace core {
    ...
    DQM_PLUGIN_DECL( DimRunControlClient , "DimRunControlClient" )

    CurrentRunRpcInfo::CurrentRunRpcInfo(char *rpcName, DimRunControlClient *pClient) :
    ...
    void CurrentRunRpcInfo::rpcInfoHandler()
    {
    ...
    DimRunControlClient::DimRunControlClient() :
		    RunControlClient(),
		    m_isConnected(false),
    ...
    StatusCode DimRunControlClient::connectToService()
    {
      ...
      m_pCurrentRunRpcInfo = new CurrentRunRpcInfo( (char *) currentRunRpcName.c_str() , this );
    ...
    StatusCode DimRunControlClient::disconnectFromService()
    {
      if(!isConnectedToService())
        return STATUS_CODE_SUCCESS;
    ...
    bool DimRunControlClient::isConnectedToService() const
    {
    ...
    StatusCode DimRunControlClient::sendStartNewRun(Run *pRun, const std::string &password)
    {
      m_pOutBuffer->reset();
      ...
      RpcResponse *response = (RpcResponse *) rpc.getData();
      ...
      RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=, RunControl::startNewRun( pRun, password ));
    ...
    StatusCode DimRunControlClient::sendStartNewRun(int runNumber, const std::string &description, const std::string &detectorName, const std::string &password)
    {
      Run *pRun = new Run( runNumber, description, detectorName );
    ...
    StatusCode DimRunControlClient::sendEndCurrentRun( const std::string &password )
    {
      m_pOutBuffer->reset();
      ...
      RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=, RunControl::endCurrentRun( password ));
    ...
    void DimRunControlClient::infoHandler()
    {
    ...
    void DimRunControlClient::handleCurrentRunRpcInfo(DimRpcInfo *pRpcInfo)
    {
      if( ! this->isConnectedToService() )
        return;
    ...
    void DimRunControlClient::configureInBuffer( char *pBuffer , uint32_t bufferSize )
    {
      if( ! m_pInBuffer )
