...
namespace dqm4hep {
  ...
  namespace core {
    ...
    // plug-in declaration
    DQM_PLUGIN_DECL( TimerCycle , "TimerCycle" )
    ...
    bool TimerCycle::isEndOfCycleReached() const
    {
      if( CoreTool::now() > this->getStartTime() + std::chrono::seconds(static_cast<unsigned int>(this->getCycleValue())) )
        return true;
