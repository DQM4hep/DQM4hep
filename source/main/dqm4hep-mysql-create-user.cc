...
int main(int argc, char* argv[])
{
  ...
  try
  {
    ...
    query << "CREATE USER IF NOT EXISTS \"DQM4HEP\"@\"%\",\"DQM4HEP\"@\"localhost\",\"DQM4HEP\"@\"" << hostName << "\";";
    ...
    query.str("");
    query << "grant all privileges on " << dbDbArg.getValue() << ".* to \"" << dbUserArg.getValue() << "\"@\"localhost\";";
    THROW_RESULT_IF(STATUS_CODE_SUCCESS, !=, interface.execute(query.str()));
    
    query.str("");
    query << "grant all privileges on " << dbDbArg.getValue() << ".* to \"" << dbUserArg.getValue() << "\"@\"" << hostName << "\";";
    THROW_RESULT_IF(STATUS_CODE_SUCCESS, !=, interface.execute(query.str()));
  ...
  {
    dqm_error( "MySQL error, couldn't create user '{0}' and permissions for database '{1}': {2}", dbUserArg.getValue(), dbDbArg.getValue(), exception.toString() );
    return 1;
  }
  
  dqm_info( "Successfully created user '{0}' and set permissions for database '{1}' on host '{2}' !", dbUserArg.getValue(), dbDbArg.getValue(), dbHostArg.getValue() );
